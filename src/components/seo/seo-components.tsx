...
export function ShareableResultCard({
  ...
  const shareText = isRtl
    ? `قمت بـ ${toolName} في ${processingTime.toFixed(1)} ثانية — بالكامل في متصفحي! 🔒 صفر رفع، 100% خاص. جربه مجاناً: myworldpdf.com`
    : `Just ${toolName.toLowerCase()}ed my PDF in ${processingTime.toFixed(1)}s — entirely in my browser! 🔒 Zero uploads, 100% private. Try it free: myworldpdf.com`;

  const shareUrl = typeof window !== "undefined" ? window.location.href : "https://www.myworldpdf.com";
    ...
    <div className="mt-6 p-5 rounded-2xl bg-gradient-to-br from-surface-50 to-surface-100/50 dark:from-surface-800/50 dark:to-surface-900/50 border border-surface-200/80 dark:border-surface-700/50">
      ...
      <div className="flex flex-wrap gap-2">
        <button
          ...
          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-[#25D366]/10 text-[#25D366] hover:bg-[#25D366]/20 transition-colors text-xs font-semibold"
        >
        ...
        <button
          onClick={() => window.open(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(shareUrl)}&quote=${encodeURIComponent(shareText)}`, "_blank", "noopener,noreferrer,width=600,height=400")}
          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-[#1877F2]/10 text-[#1877F2] hover:bg-[#1877F2]/20 transition-colors text-xs font-semibold"
        >
